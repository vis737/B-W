...
async function syncAdminConfigFromSupabase() {
  const targetUser = process.env.ADMIN_USERNAME || 'admin';
  const targetPass = process.env.ADMIN_PASSWORD || 'meriseshop_2025';
  const hashedPass = bcrypt.hashSync(targetPass, 12);
  const config = { username: targetUser, password: hashedPass };
  ...
  if (supabase) {
    try {
      await supabase.from('admin_config').upsert({ username: targetUser, password: hashedPass });
      console.log('◇ Synced administrative credentials with Supabase.');
    } catch (err) {
...
function readAdminConfig() {
  ...
  return {
    ...
    password: defaultPass || 'meriseshop_2025'
  };
}
...
// Trust reverse proxy headers (Render, Railway, Cloudflare, Heroku)
...
function readCustomersDb(): any[] {
  try {
    if (!fs.existsSync(CUSTOMERS_FILE_PATH)) {
      fs.writeFileSync(CUSTOMERS_FILE_PATH, JSON.stringify([], null, 2));
      return [];
    }
...
function writeCustomersDb(customers: any[]) {
  try {
    fs.writeFileSync(CUSTOMERS_FILE_PATH, JSON.stringify(customers, null, 2));
  } catch (error) {
    console.error('Error writing customers database:', error);
  }
}

app.post('/api/login-customer', rateLimiter(50, 15 * 60 * 1000), async (req, res) => {
  try {
    const email = sanitizeEmail(req.body?.email);
    ...
    let customers = readCustomersDb();
    let customer = customers.find((c: any) => c.email.toLowerCase() === email.toLowerCase());

    // Fallback: check Supabase if customer not found locally
    if (!customer && supabase) {
      try {
        const { data: supabaseCustomer } = await supabase.from('customers').select('*').eq('email', email.toLowerCase()).maybeSingle();
        if (supabaseCustomer) {
          customer = {
            id: supabaseCustomer.id,
            email: supabaseCustomer.email,
            name: supabaseCustomer.name,
            passwordHash: supabaseCustomer.password_hash || supabaseCustomer.passwordHash
          };
          customers.unshift(customer);
          writeCustomersDb(customers);
        }
      } catch (err) {
        // Continue if Supabase lookup fails
      }
    }

    if (!customer) {
      return res.status(401).json({ error: 'Invalid email or password. Please check your credentials or create a new account.' });
    }

    const bcrypt = await import('bcryptjs');
    if (!customer.passwordHash || !bcrypt.compareSync(password, customer.passwordHash)) {
...
app.post('/api/register-customer', rateLimiter(50, 60 * 60 * 1000), async (req, res) => {
  try {
    const email = sanitizeEmail(req.body?.email);
    ...
    if (customers.some((c: any) => c.email.toLowerCase() === email.toLowerCase())) {
    ...
    if (supabase) {
      try {
        ...
        // Continue if Supabase table is not yet created
      }
    }
...
// Contact Us Form Submission Endpoint – Dispatches to meriseshop.2025@gmail.com
app.post('/api/contact', rateLimiter(10, 15 * 60 * 1000), async (req, res) => {
  try {
    const name = sanitizeString(req.body?.name, 100);
    const email = sanitizeEmail(req.body?.email);
    const phone = sanitizeString(req.body?.phone || '', 30);
    const subjectText = sanitizeString(req.body?.subject || 'General Inquiry', 150);
    const message = sanitizeString(req.body?.message || '', 3000);

    if (!name || !email || !message) {
      return res.status(400).json({ error: 'Name, email, and detailed message are required.' });
    }

    const recipient = 'meriseshop.2025@gmail.com';
    const emailSubject = `📬 New Concierge Inquiry from ${name}: ${subjectText}`;
...
<head>
  <meta charset="utf-8">
  <title>New Contact Inquiry - MERIS E-SHOP</title>
</head>
<body style="font-family: Arial, sans-serif; background-color: #f8fafc; padding: 20px 0;">
  <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border: 1px solid #e2e8f0; border-radius: 20px; overflow: hidden;">
    <div style="background: linear-gradient(135deg, #0f172a 0%, #1e293b 100%); padding: 32px 24px; text-align: center; border-bottom: 4px solid #c5a021;">
      <h1 style="color: #c5a021; margin: 0; font-size: 24px; font-weight: 700; letter-spacing: 2px;">MERIS E-SHOP</h1>
      <p style="color: #94a3b8; margin: 6px 0 0 0; font-size: 11px; letter-spacing: 2px; text-transform: uppercase;">Concierge Support Request Received</p>
    </div>
    <div style="padding: 28px 24px;">
      <h2 style="font-size: 16px; color: #0f172a; margin-top: 0; border-bottom: 2px solid #f1f5f9; padding-bottom: 12px;">Contact Form Submission Details</h2>
      <table style="width: 100%; font-size: 13px; color: #334155; border-collapse: collapse; margin-top: 16px;">
        <tr>
        ...
        <tr>
          ...
          <td style="padding: 8px 0;"><a href="mailto:${email}" style="color: #c5a021; text-decoration: none; font-weight: bold;">${email}</a></td>
        </tr>
        ${phone ? `
        <tr>
        ...
        <tr>
          ...
          <td style="padding: 8px 0; color: #0f172a;">${subjectText}</td>
        </tr>
        <tr>
      ...
      <div style="margin-top: 20px; padding: 16px; background-color: #f8fafc; border-left: 4px solid #c5a021; border-radius: 8px;">
        <h3 style="margin: 0 0 8px 0; font-size: 12px; font-weight: bold; color: #64748b; text-transform: uppercase;">Message Content:</h3>
    ...
    <div style="background-color: #f8fafc; border-top: 1px solid #f1f5f9; padding: 20px; text-align: center;">
      <p style="font-size: 11px; color: #94a3b8; margin: 0;">
        Meris Artisanal Studio Co. • Headquarters: Azhagappapuram, Tamil Nadu
      </p>
    </div>
  </div>
</body>
    ...
    // Send real SMTP email to meriseshop.2025@gmail.com
    if (smtpEmailConfigured()) {
      try {
        const transporter = nodemailer.createTransport({
          host: process.env.SMTP_HOST,
        ...
        await transporter.sendMail({
          from: `"${name} (via Meris Store)" <${process.env.SMTP_FROM_EMAIL || process.env.SMTP_USER}>`,
          to: recipient,
          replyTo: email,
          subject: emailSubject,
          html: htmlContent
        });
        console.log(`[Contact Form] Inquiry email successfully dispatched to ${recipient}.`);
      } catch (smtpErr) {
        console.error('[Contact Form] SMTP dispatch failed:', smtpErr);
      }
    } else {
      console.log(`[Contact Form] Simulated dispatch to ${recipient}:`, emailSubject);
    }
...
// --- PAYU PAYMENT GATEWAY PREPARATION ENDPOINTS ---
app.post('/api/payu/hash', rateLimiter(20, 15 * 60 * 1000), (req, res) => {
  try {
    ...
    const merchantKey = process.env.PAYU_MERCHANT_KEY || 'PAYU_TEST_KEY';
    const merchantSalt = process.env.PAYU_MERCHANT_SALT || 'PAYU_TEST_SALT';

    if (!txnid || !amount || !productinfo || !firstname || !email) {
      return res.status(400).json({ error: 'Missing required PayU parameters (txnid, amount, productinfo, firstname, email).' });
    }

    const cleanAmount = parseFloat(amount).toFixed(2);
    const hashSequence = `${merchantKey}|${txnid.trim()}|${cleanAmount}|${productinfo.trim()}|${firstname.trim()}|${email.trim()}|${udf1 || ''}|${udf2 || ''}|${udf3 || ''}|${udf4 || ''}|${udf5 || ''}||||||${merchantSalt}`;
    ...
    res.json({
      ...
      actionUrl: (process.env.PAYU_ENV === 'production')
...
app.post('/api/payu/verify', (req, res) => {
  try {
    ...
    if (!payload || !payload.hash || !payload.status) {
      return res.status(400).json({ error: 'Invalid PayU response payload' });
    }

    const status = (payload.status || '').trim();
    const udf5 = (payload.udf5 || '').trim();
    const udf4 = (payload.udf4 || '').trim();
    const udf3 = (payload.udf3 || '').trim();
    const udf2 = (payload.udf2 || '').trim();
    const udf1 = (payload.udf1 || '').trim();
    ...
    const reverseHashSequence = `${merchantSalt}|${status}||||||${udf5}|${udf4}|${udf3}|${udf2}|${udf1}|${email}|${firstname}|${productinfo}|${amount}|${txnid}|${key}`;
    const calculatedHash = crypto.createHash('sha512').update(reverseHashSequence).digest('hex');

    const isValid = calculatedHash.toLowerCase() === (payload.hash || '').toLowerCase();
...
app.post('/api/orders', rateLimiter(10, 15 * 60 * 1000), async (req, res) => {
  try {
    ...
    // Handle Cash on Delivery order defaults
    const isCod = newOrder.paymentMethod?.toLowerCase().includes('cash on delivery') || newOrder.paymentMethod?.toUpperCase() === 'COD';
    if (isCod) {
...
app.post('/api/admin/login', rateLimiter(30, 15 * 60 * 1000), (req, res) => {
  try {
